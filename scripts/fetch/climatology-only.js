...
import { climatologyIsFresh, fetchOpenMeteoClimatology } from './adapters/openmeteo-climatology.js';
...
async function main() {
  ...
  const maxLocs = Number(process.env.CLIMATOLOGY_MAX_LOCS || stale.length) || stale.length;
  console.log(
    `climatology-only: ${stale.length}/${locations.length} need refresh; fetching up to ${maxLocs}`,
  );
  ...
  if (result.status === 'error' && written === 0) {
    process.exitCode = 1;
  }
}
