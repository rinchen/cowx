...
- **Stale watchdog:** `check-stale-data.yml` every 30 minutes (`*/30 * * * *`), two tiers on production `generatedAt`:
  ...
  - **≥ `NOTIFY_MINUTES` (120):** dispatch Update Weather, then **wait** (~20m) for live `meta.json` to recover (age under 90m, or `generatedAt` newer than the baseline). Discord-alert (when `NOTIFY_WEBHOOK_URL` is set) and fail the run only if still unrecovered after the wait **and** no Update Weather run is in progress or queued (a full fetch+deploy can exceed the wait window). Unreachable / unparseable live `meta.json` alerts immediately (no wait).
