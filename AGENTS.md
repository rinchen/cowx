...
- **Stale watchdog:** `check-stale-data.yml` every 30 minutes (`*/30 * * * *`), two tiers on production `generatedAt`:
  - **≥ `STALE_MINUTES` (90):** **dispatch** `update-weather.yml` (self-heal) and stay green — no Discord, unless an Update Weather run is already in progress or queued (do not stack a second run; concurrency would cancel the previously queued one). GitHub often delays weather crons by 1–3 hours; without this catch-up the CDN stays frozen.
  - **≥ `NOTIFY_MINUTES` (120):** dispatch Update Weather (same skip-if-busy rule), then **wait** (~20m) for live `meta.json` to recover (age under 90m, or `generatedAt` newer than the baseline). The wait does a **final poll** after timeout so a deploy that lands during the last sleep is not missed. Discord-alert (when `NOTIFY_WEBHOOK_URL` is set) and fail the run only if still unrecovered after the wait **and** no Update Weather run is in progress, queued, or newly succeeded (last ~10m — Pages/CDN can lag the workflow conclusion). Unreachable / unparseable live `meta.json` alerts immediately (no wait).
...
When the **weather fetch step** in `update-weather.yml` fails (`weather_fetch`), or the **gh-pages deploy / Pages build verification** fails, a notify job POSTs a summary to `NOTIFY_WEBHOOK_URL` (if set). Install/push races after a successful fetch do **not** notify. Separately, `check-stale-data.yml` auto-dispatches Update Weather at ≥ 90 minutes **silently** (unless one is already running or queued), and at ≥ 120 minutes waits for recovery before POSTing (or immediately if live `meta.json` is unreachable) — i.e. only when self-heal did not land and no Update Weather run is in progress, queued, or just succeeded. Neither opens a GitHub Issue. Never log or echo the webhook URL. See `how-it-works.html` for the user-facing explanation.
