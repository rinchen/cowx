...
Weather data targets a **~45-minute** cadence via `.github/workflows/update-weather.yml` (staggered `:05/:15/:25/:35/:45/:55` crons plus `workflow_dispatch`). GitHub may delay schedules; the workflow **skips** fetch/deploy when live CDN `meta.json` is still fresh (< 40 minutes), and can **deploy-only** when `main` is fresh but the CDN is stale. Every **30 minutes**, `.github/workflows/check-stale-data.yml` checks live `generatedAt`; if it is ≥ 90 minutes old, it **dispatches Update Weather** (self-heal), Discord-alerts when `NOTIFY_WEBHOOK_URL` is set, and fails the run. Fetch/Pages verify failures also notify. Committed JSON in `public/data/` (published by the weather workflow) is what visitors see between runs. `public/.nojekyll` keeps GitHub Pages from running Jekyll on the static tree.
